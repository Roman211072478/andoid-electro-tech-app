...
import com.fiki.n3.technology.electro.electrotechn3application.factories.chain.factories.rope.method.RopeBreakCUDChainFactory;
 ...
 */
public class RopeBreakCUDService extends IntentService {

    private ResultDTO resultDTO;
    private Bundle bundle;
    private ContextDTO ctx;

    public RopeBreakCUDService() {
    ...
    private void runService(Intent intent)
    {
        ...
        RopeBreakDTO ropeBreakDTO;

        ResultReceiver receiver = intent.getParcelableExtra("receiverTag");//send ur receive through
        ropeBreakDTO = (RopeBreakDTO)intent.getSerializableExtra("dtoTag");
        ...
        if(!request.isEmpty())
        {
            String sResult = RopeBreakCUDChainFactory.performRequest(request, ropeBreakDTO,ctx);//returns primary key or -1 for unsuccessFul
